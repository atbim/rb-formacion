export const crearSpritesAlertas = async (viewer) => {
  ...
  //const spriteIconUrl = `${baseURL}PHOTO.jpg`;
/*
...
/*
  ...
  viewableData.spriteSize = 100; // Sprites as points of size 24 x 24 pixels
...
*/
  ...
  const grafico = document.getElementById('cmdAlertas');
  grafico.onclick = () => {
    var milado =  document.getElementById('mySidebar')
    if (milado.hidden == true)
      {
       milado.hidden = false
       grafico.innerText = 'Close Real Time' 
       viewer.resize()
     }
     else {
      milado.hidden = true
      grafico.innerText = 'Real Time'
      viewer.resize()
